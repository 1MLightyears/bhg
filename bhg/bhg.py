...
class BHG(object):
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.__succeed = (CLI_Entrance) and (self.CLIEntrance(*args, **kwargs) == 0)

    def __str__(self):
        if self.__succeed:
            return ''
        else:
            return self.CLIEntrance.__doc__  #Fire的--help等于print(self.__str__())
...
--savepng :可选参数，若指定该选项则会保存图片为png格式在当前目录下，
    ...
    def text(self, s = '',font_size=0,lang='zh'):
        ...
        '''
        def ToList(v):
            v = list(v) if isinstance(v, tuple) else v if isinstance(v, list) else[v]
            return v
        ...
        for i in range(len(s)):
            ...
            if i < len(lang):
                ...
                if not exists(font_dict[this_lang]):
                    stderr.write('语种错误:名为' + this_lang + '的语种缺失字体文件' + font_dict[this_lang]+'\n')
                    return 4
            #添加一行
    ...
    def make(self):
        ...
        for i in range(len(self.__setting['text'])):
            if self.__setting['fontsize'][i] == 0:  #如果没有手动设定字号
                self.__setting['fontsize'][i] = round(self.__image.size[0] / len(self.__setting['text'][i]))
        ...
        text_height = round(sum(self.__setting['fontsize']) * 1.3)  #字体画布高，空出行间距
        text_canvas = Image.new('RGBA', (text_width, text_height), self.__setting['bgcolor'])
        ...
        top = 0
        for i in range(len(self.__setting['text'])):
            fnt = ImageFont.truetype(self.__setting['font'][i], self.__setting['fontsize'][i], encoding="unic")  #设置字体
            width, height = fnt.getsize(self.__setting['text'][i])
            left = round(text_width / 2 - width / 2)
            if left < 0:
                stderr.write("提示：第"+str(i+1)+"行文字过多,可能无法显示完整\n")
            text_drawer.text((left, top), self.__setting['text'][i], font=fnt, fill=text_color[:3])
            top += round(height*1.1)  #留出行间距

        #创建一个新的空白画布，将原图和字体图贴上去
        self.__result = Image.new('RGBA',(text_width, text_height + self.__image.size[1]),self.__setting['bgcolor'])
        self.__result.paste(self.__image,(0,0))
        self.__result.paste(text_canvas, (0,self.__image.size[1],text_width,text_height + self.__image.size[1]))
    ...
    def list_setting(self, human_readable=False):
        ...
        if human_readable:
            ...
            if self.__setting['pathtype'] == 'local':
                ...
                print('网络图片')
            for i in range(len(self.__setting['text'])):
                print(i+1, '."', self.__setting['text'][i],'"',sep='',end=' ')
                print('字号:',end='')
                ...
                if i >= len(self.__setting['lang']):
                    ...
                    print(self.__setting['lang'][i],'字体文件:',font_dict[self.__setting['lang'][i]],)
